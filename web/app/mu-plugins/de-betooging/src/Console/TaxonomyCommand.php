...
class TaxonomyCommand extends GeneratorCommand
{
     ...
     * @var string
     */
    protected $signature = 'de-betooging:taxonomy {name : taxonomy} {post_type : the post type}';
    ...
    public function handle()
    {
        ...
        if (get_post_type_object($postTypeSlug) === null) {
            $this->error(sprintf(
                'Post type does not exist. Please add the post type "%s" first: wp acorn de_betooging:post-type %s',
                $postTypeSlug,
    ...
    protected function getPath($name)
    {
        ...
        return app('de_betooging.base_path') . '/Taxonomies/' . $postType . '/' . $fileName;
    }
}
